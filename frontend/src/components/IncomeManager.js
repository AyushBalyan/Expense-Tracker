...
const IncomeManager = () => {
  ...
  const { addIncome, lockIncome, state, logout } = useContext(AppContext);
  ...
  const handleSubmit = (e) => {
    ...
    if (amount && month && year) {
      addIncome(parseFloat(amount), parseInt(month), parseInt(year));
      setAmount("");
      setMonth("");
      setYear("");
    }
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          ...
          <div>
            ...
            <div className="space-y-4">
              {state.incomeHistory.map((income) => (
                <div
                  key={income.id}
